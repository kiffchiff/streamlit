...
def display_dataframe(df, n=10, styled=False):
    ...
    if styled:
        st.dataframe(df.head(n).style.map(highlight_counts, subset=['횟수']))
    else:
...
블랙리스트_데이터['일시'] = pd.to_datetime(블랙리스트_데이터['일시'])

with st.sidebar:
    choose = option_menu('서울교통공사', ['역선택', '영상', '블랙리스트'],
...
if choose == '역선택':
    ...
    if 'removed_cards' not in st.session_state:
        st.session_state.removed_cards = pd.DataFrame(columns=블랙리스트_데이터.columns)
    station = list(df['역명'].unique())
    selected_station = st.sidebar.selectbox('역을 선택하세요', station)
    ...
    blacklist_df = 블랙리스트_데이터[블랙리스트_데이터['일시'] <= pd.to_datetime(selected_date)]
    ...
    if 'removed_cards' not in st.session_state:
        st.session_state.removed_cards = pd.DataFrame(columns=blacklist_df.columns)
    ...
    if st.button('제거'):
        if cards_to_remove:
            ...
            블랙리스트_데이터.drop(블랙리스트_데이터[블랙리스트_데이터['카드식별번호'].isin(cards_to_remove)].index, inplace=True)
    ...
    if st.button('복원'):
        if restore_cards:
            ...
            블랙리스트_데이터 = pd.concat([블랙리스트_데이터, restored_cards])
